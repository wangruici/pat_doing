...
//print_node(node,valid_count+removed_count);
	...
	printf("%05d %d -1\n",node[valid_count-1].address,node[valid_count-1].data);

	for(int i=valid_count;i<valid_count+removed_count-1;++i){
		printf("%05d %d %05d\n",node[i].address,node[i].data,node[i+1].address);
	}
	if(removed_count!=0)
	{
		printf("%05d %d -1\n",node[valid_count+removed_count-1].address,node[valid_count+removed_count-1].data);
	}
	return 0;
}
