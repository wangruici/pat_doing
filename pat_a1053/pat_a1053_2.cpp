	...
	while(!node_stack.empty()){
		if(flag[node_stack.top().index]==false){
			...
			for(int i=0;i<(int)node.children.size();++i){
				...
				if(nodes[node.children[i]].weights_sum==object&&nodes[node.children[i]].children.size()<=0){
					sub_ans.push_back(nodes[node.children[i]].weight);
					ans.push_back(sub_ans);
					sub_ans.pop_back();
				}
				else if(nodes[node.children[i]].weights_sum>=object){
					;
				}
				else{
